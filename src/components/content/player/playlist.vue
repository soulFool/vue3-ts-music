<template>
  <teleport to="body">
    <transition name="list-fade">
      <div class="playlist" v-show="visible && store.playlist.length" @click="hide">
        <div class="list-wrapper" @click.stop>
          ...
          <scroll class="list-content" ref="scrollRef">
            <transition-group ref="listRef" name="list" tag="ul">
              <li class="item" v-for="song in store.sequenceList" :key="song.id" @click="selectItem(song)">
...
<script lang="ts">
import { defineComponent, ref, watch, nextTick, DefineComponent } from 'vue'
...
export default defineComponent({
  ...
  setup() {
    ...
    const getCurrentIcon = (song: ISingerDetail) => {
      if (song.id === (store.currentSong as ISingerDetail).id) {
        return 'icon-play'
      }
    }
    ...
    const selectItem = (song: ISingerDetail) => {
      const index = store.playlist.findIndex((item: ISingerDetail) => {
    ...
    const scrollToCurrent = () => {
      const index = store.sequenceList.findIndex((song: ISingerDetail) => {
        return (store.currentSong as ISingerDetail).id === song.id
      })
      if (index === -1) {
        return
      }
      const target = listRef.value!.$el.children[index] as HTMLElement

      scrollRef.value!.scroll!.scrollToElement(target, 300, false, false)
    }

    const removeSong = (song: ISingerDetail) => {
      if (removing.value) {
        return
      }
      removing.value = true
